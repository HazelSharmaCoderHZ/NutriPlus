...
export default function CalorieCounter() {
  ...
  const fetchCalories = async () => {
    ...
    try {
      const res = await fetch(
        ...
        {
          headers: {
            "X-Api-Key": process.env.NEXT_PUBLIC_CALORIE_API_KEY, // store key in .env.local
          },
        }
      );
      ...
      if (data.items && data.items.length > 0) {
        setResult(data.items[0]); // take first match
      } else {
  ...
  return (
    <main className="min-h-screen flex flex-col md:flex-row items-center justify-center  p-6">
      ...
      {/* Right Side - Calorie Counter */}
      <div className="w-full md:w-1/2 flex flex-col items-center">
        <h1 className="text-5xl text-white font-bold mb-6">🍎 Calorie Counter</h1>
        ...
        {result && (
          <div className="mt-6 border bg-gradient-to-r from-indigo-400 via-purple-400 to-yellow-200 p-4 rounded-lg hover:scale-105 text-left w-80">
            ...
            <p>
              Calories:{" "}
              <span className="text-yellow-300">{result.calories}</span>
            </p>
